...
describe('MultirequestAsyncController', () => {
  it('should handle multiple requests resolving in order', async () => {
    ...
    const controller = multiRequestConcurrencyController(maxConcurrency)
    ...
    const results = await Promise.all(
      requests.map((request) => controller(request, 1)),
    )
  ...
  it('should handle light weight requests', async () => {
    ...
    const controller = multiRequestConcurrencyController(maxConcurrency)
    const results = await Promise.all(
      requests.map((request, i) => controller(request, weights[i])),
    )
  ...
  it('should respect order of execution (using ensureOrder flag)', async () => {
    ...
    const controller = multiRequestConcurrencyController(maxConcurrency, true)
    const results = await Promise.all(
      requests.map((request, i) => controller(request, weights[i])),
    )
