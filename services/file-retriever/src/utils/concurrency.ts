...
export const multiRequestConcurrencyController = (
  ...
  const findExecutableTask = () => {
    const index = queue.findIndex(
      (e) => active + e.concurrency <= maxConcurrency,
    )
    ...
    console.log(
      `Found task with index ${index} and concurrency ${queue[index].concurrency}`,
    )
  ...
  const manageJobFinalization = async (concurrency: number) => {
    ...
    if (executableTask) {
      setImmediate(() =>
        internalRunJob(executableTask.job, executableTask.concurrency),
      )
    }
  }

  const internalRunJob = async <T>(
    job: Job<T>,
    concurrency: number,
  ): Promise<T> => {
    active += concurrency
